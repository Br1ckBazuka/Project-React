...
class ArticleComponent extends Component {
	...
	componentDidMount() {
		...
		const fetchUrls = async () => {
			...
			await this.postQuery('http://wp.ru/wp-json/wp/v2/users/'+this.state.authorId).then(response => {
				if(this.state.isJson){
					this.setState({ author: response.name });
					this.setState({ authorImage: response.avatar_urls[48] });
				}
			})
		};
		fetchUrls();
			...
			<div className="article_header">
				<div className="article_header_left">
					<div className="author">
						<div className="author_image"><img alt="" src={this.state.authorImage} /></div>
						{ this.state.author === "" ? <span>Загрузка автора...</span> : <span>Автор <b>{this.state.author}</b></span> }
					</div>
				</div>
				<div className="article_header_center">
					{ this.state.title === "" ? <span>Загрузка заголовка...</span> : <span>{ this.state.title }</span> }
				</div>
